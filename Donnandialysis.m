function Y = Donnandialysis(x, x0, np, dx, X_poly, Di_Na, Di_Cl, c_di_0,c_con_0,  z_Na, z_Cl)
    ...
    for i = 2:np
        ...
        Y(1*(np+1)+i) =  -Di_Cl* ( x(1*(np+1)+i+1)-x(1*(np+1)+i-1))/2/dx - z_Cl*Di_Cl* x(1*(np+1)+i)*(x(2*(np+1)+i+1)-x(2*(np+1)+i-1))/2/dx - x(3*(np+1)+3);
    end
    for i = 1:np+1
    ...
    Y(np+2) = c_con_0 - x(0*(np+1)+np+1) *exp(-z_Na*(x(3*(np+1)+1)-x(2*(np+1)+np+1)));
    Y(2*np+2) = c_con_0 - x(1*(np+1)+np+1) *exp(-z_Cl*(x(3*(np+1)+1)-x(2*(np+1)+np+1)));  
    ...
    Y(3*(np+1)+2) = x(3*(np+1)+2) - x(3*(np+1)+3);
end
