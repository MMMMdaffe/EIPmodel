function Y = spacer_unit1(x, x0, charging, np, dx,dsp,  daem, dTT,X_poly, V_T, F, C_St_vol, p_pore, p_IEP, Di_Na, Di_Cl,Di_Na_b, Di_Cl_b, I_max, L_elec, c_di_0,c_con_0,  Alfa, z_Na, z_Cl,E,tau_sp, kk, j_Na_left_flux, j_Na_right_flux,j_Cl_left_flux, j_Cl_right_flux)
    Y = zeros(9*(np+1)+6, 1);
...
%% AEM1
    ...
    for i = 1:np+1
        ...
        elseif charging == 2    
            Y(9*(np+1)+3)    = x(9*(np+1)+1) - x(9*(np+1)+2) - (j_Na_right_flux - j_Cl_right_flux);
